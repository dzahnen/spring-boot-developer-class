...
public class CrossSnippetLanguageCodeRepository implements SimpleRepository<CrossSnippetLanguageCode> {
	...
	@Override
	public void save(Collection<CrossSnippetLanguageCode> items) {
		items.forEach(cross -> {
			this.save(cross);
		});
	}
	...
	@Override
	public CrossSnippetLanguageCode save(CrossSnippetLanguageCode item) {
		assert item.getSnippetId() != null;
		assert item.getLanguageId() != null;
		assert item.getCodeId() != null;

		CrossSnippetLanguageCode cross = this.findOne(item.getSnippetId());
	...
	@Override
	public CrossSnippetLanguageCode findOne(String id) {
		try {
