...
public class SnippetRepository implements SimpleRepository<Snippet> {
	...
	@Override
	public void save(Collection<Snippet> items) {
		items.forEach(code -> this.save(code));
	}
	...
	@Override
	public Snippet save(final Snippet item) {
		assert item.getId() != null;
		assert item.getTitle() != null;
		assert item.getLang() != null;
		assert item.getCode() != null;

		Snippet snippet = this.findOne(item.getId());
		...
		if (snippet == null) {
			...
			this.crossRepo.save(new CrossSnippetLanguageCode(item.getId(), item.getLang().getId(), item.getCode().getId()));
			this.langRepo.save(item.getLang());
			this.codeRepo.save(item.getCode());
			...
			this.crossRepo.save(new CrossSnippetLanguageCode(snippet.getId(), snippet.getLang().getId(), snippet.getCode().getId()));
			this.langRepo.save(item.getLang());
			this.codeRepo.save(item.getCode());
